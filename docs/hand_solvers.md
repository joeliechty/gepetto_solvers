...
|---|---|---|
| Pose the hand from tendon tensions (no contact) | `HandSolver` (no contact env) → `fk_5f_sweep.py` | §1.1 |
| Put fingertips on an object, one shot | `HandSolver` + contact env → `ik_5f_contact.py` | §1.3, §1.4 |
| Keep fingers out of the object / each other | any solver + `attach_collision` | §1.5 |
| Plan a K+1-step grasp trajectory | `HandTrajectoryPlanner` → `traj_5f_contact.py` | §1.4 |
...
gepetto_solvers/
  include/gepetto_solvers/     public C++ headers
    ...
    digits/tendon/         TendonFingerModel<N>  — one finger: rod + tendon disc
    ...
    hand/                  the hand, and nothing about what KIND of hand
      HandKinematics.h  the interface a mechanism implements: its factors, and
                        site_pose_key — the only thing the task constraints ask
      HandKinematicsRegistry.h  load a kinematics by name
      HandSpec.h        one hand described for HandModel: the per-digit TASK env,
                        the opposing digit, and an opaque kinematics payload
      HandState.h       a solved hand, key-independent
      ConstraintTagger.h  the one way a hard constraint may enter the graph
      HandModel.h    the graph builder — shared by both solvers below
      HandSolver.h   single-shot static / IK solve
      HandTrajectoryPlanner.h  K+1-step trajectory (§1.4–1.6)
      kinematics/tendon/  TendonHandKinematics — the tendon hand, registered
                          as "tendon". One directory per mechanism.
    utils/
  ...
  src/                         C++ implementation
    cosserat_rod/  digits/tendon/  measurement/  utils/
    hand/
      HandModel.cpp  construction (loads the kinematics), the contact-form rule
      HandGraph.cpp  build_graph — §3.2, and its insertion order matters
      HandValues.cpp initial values: the TASK half (object pose, witnesses)
      ConstraintTagger.cpp  add_eq / add_ineq
      HandKinematicsRegistry.cpp  the registry, and HandSpec validation
      kinematics/tendon/TendonHandKinematics.cpp  the tendon mechanism: the
                          per-digit construction, the rod+tendon factors, the
                          values/state round trip and the GP temporal priors
...
The layering is strict: `HandSolver` and `HandTrajectoryPlanner` are
both thin `SolverBase` subclasses. They own `HandModel` instances and
differ almost entirely in **how many models they build** and **what extra
factors they staple on**.

And one layer further down: `HandModel` owns a `HandKinematics`, loaded by name
from the `HandSpec`, and never learns what kind of mechanism it is. It addresses
the hand only through `site_pose_key({digit, node})`, so a hand built from
separate digits and one defined as a single whole mechanism are the same thing
to the graph builder. See [adding_a_hand.md](adding_a_hand.md).
...
| `W` | **shared wrist base pose** | `HandModel` | `Symbol('W', step)` |
| `O` | object pose | `HandModel` | `Symbol('O', step)` |
| `Y` | object witness point p_c,obj | `HandModel` | `Symbol('Y', finger_index)` |
| `U` | table witness point p_c,table | `HandModel` | `Symbol('U', 1000*step + finger)` |
...
`HandModel`'s constructor calls `fp->set_hand_base(offset, shared_wrist_key)`
...
AL state in and out (`HandSolver`): `reset_al_duals()` drops the carried
...
### 3.2 `HandModel` — the graph builder (not a solver)
...
### 3.3 `HandSolver` — single-shot static / IK (§1.3, §1.4)

One `HandModel` at step 0. `solve(tensions, tip_wrenches)`.
...
solver = gepetto_solvers.HandSolver(configs, cfg)
...
### 3.4 `HandTrajectoryPlanner` — K+1 steps (§1.4.3, §1.5.2, §1.6.2)

Owns **K+1 `HandModel` instances**, one per step, each with its own
...
| `object_contact_center_direct` | forces the witness-free object equality — already the **default** for any ellipsoid contact in `HandModel` | Eq 1.108 |
...
| Class | Wraps | Mirrors |
|---|---|---|
| `HandFKSolver` | `HandSolver`, no contact | `fk_5f_sweep.py` |
| `HandIKSolver` | `HandSolver` + contact, one shot | `ik_5f_contact.py` |
| `HandIKStepper` | `HandSolver` + contact, one AL outer iteration per call | `viz_interactive.py`, `debug_ik_step_trace.py` |
| `HandPlannerSolver` | `HandTrajectoryPlanner` | `traj_5f_contact.py`, `traj_5f_slide_grasp.py` |
...
`HandState` behind a frame — the form `HandSolveParams.initial_state`
...
|---|---|---|
| The posture | `HandSolveParams.initial_state` → `HandSolverConfig.initial_state` | whole hand back to the straight-rod, Q = 0 guess |
...
`HandSolverConfig.initial_state` (gated on `capabilities()["solver_seed"]`)
...
`HandModel::build_graph` therefore emits a **semantic tag** for every hard
...
`HandSolver`, then per frame commands a sinusoidal wrist pose through
...
is the fix, not a symptom to tune around.** `HandModel::get_initial_values`
...
2. `src/hand/HandGraph.cpp` — `build_graph`: every factor in the
...
```
hand/  HandModel · HandSolver · HandTrajectoryPlanner
      │       HandModel.cpp / HandGraph.cpp / HandValues.cpp -- one class, three
      │       translation units; HandGraph.cpp is build_graph and its insertion
      │       order is load-bearing (see §3.2 and tests/core/test_constraint_tags.py)
      │
      ├── HandKinematics.h  the mechanism interface. HandModel holds one, loaded
      │   by name from HandSpec, and asks it only for pose keys and factors --
      │   which is what makes the graph builder above hand-agnostic.
      │
      ├── kinematics/tendon/  TendonHandKinematics -- registered as "tendon".
      │   Owns the per-digit TendonFingerModel<N> variant, the rod+tendon factor
      │   emission, the values/state round trip and the GP temporal priors. All
      │   of it used to be inline in the graph builder.
      │
      ├── digits/tendon/  TendonFingerModel · TendonDiscWrenchFactor ·
      │                   TendonLengthFactor
      ...
      └── src/bindings/   module.cpp   ← the PYBIND11_MODULE entry point
                          ...
                          bind_digits_tendon.cpp     ← EnvironmentConfig,
...
| `src/tendon_robot/` | 1353 | whole module — its four routing bindings moved to what is now `bindings/bind_digits_tendon.cpp` first |
| `src/cosserat_rod/CosseratRodSolver.{h,cpp}` | 205 | the rod as a standalone solver; the hand uses `CosseratRodModel` directly |
| `src/digits/tendon/TendonFingerEstimatorModel.*`, `TendonFingerIterativeSolver.*`, `KnuckleBendFactor.*` | 902 | the state-estimation path |
| `src/digits/tendon/TendonFingerTrajectoryPlanner.*`, `TensionLimitFactor.h` | 817 | only `traj_1f_contact.py`, also removed |
| `src/digits/tendon/TendonFingerSolver.cpp` | 472 | only `ik_1f_contact.py`, also removed |
...
* `TendonFingerMarginals` and `TendonConfig` reach Python through
  `HandState::fingers` and `core.solvers`'s `fm.tendon_config.disc_pose_idx`,
...
**Constraint insertion order in `HandGraph.cpp` is load-bearing.** The
...
python/gepetto_solvers/__init__.py   re-exports the compiled extension
  ...
  core/                  foundational; imports nothing from the tiers below
    ...
    plotting/            utils · cosserat_rod_plotter · hand_plotter ·
...
`digits/tendon/config.py` is now `core/hands/tendon_5f/finger_config.py`, and the logging
half of `digits/tendon/utils.py` was folded into what is now `core/diagnostics.py`
...
`HandTrajectoryPlanner`; it does not call this. Keep it as the reference for
...
from a **globally auto-incrementing `next_id_`**, so each step's `HandModel`
