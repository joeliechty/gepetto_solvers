...
conda create -n crest_py12 -c conda-forge --override-channels python=3.12 -y
conda activate crest_py12

# Refuse to continue if we are not actually inside crest_py12 -- otherwise every
# $CONDA_PREFIX below silently points at the base env and pollutes it.
if [ "$(basename "${CONDA_PREFIX:-}")" != "crest_py12" ]; then
    echo "ERROR: expected to be in the 'crest_py12' env, but CONDA_PREFIX='${CONDA_PREFIX:-<unset>}'" >&2
    exit 1
fi
...
if [ "$(command -v python)" != "$CONDA_PREFIX/bin/python" ]; then
    echo "ERROR: python is not the crest_py12 interpreter" >&2
    exit 1
fi
