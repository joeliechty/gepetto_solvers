...
HandTrajectoryPlanner::HandTrajectoryPlanner(
    ...
    config_(config)
{
    ...
    // Per-step spec transform. Two independent constraint schedules ride on the
    // same per-digit env. Only the TASK half of the spec is rescheduled -- the
    // kinematics payload is shared by every step untouched, which is both correct
    // (the mechanism does not change between timesteps) and what makes this
    // schedule readable: it is visibly a statement about constraints, not about
    // the hand.
    //
    ...
    //      get_initial_values guards in HandModel.
    //
    //  (b) Support-plane phases (Section 1.6.2) about config_.k_touch, per digit
    ...
    auto make_step_spec = [&](int k) {
        const bool is_start = (k == 0);
        const bool is_goal  = (k == K);
        gepetto_solvers::HandSpec step = spec;   // shares the kinematics payload
        for (size_t i = 0; i < step.env.size(); ++i) {
            if (step.env[i].has_value()) {
                auto& env = *step.env[i];
                ...
                // digit-digit for the same reason as digit-object (see the note
                // above): a start that is already in contact with itself would
                // make k=0 infeasible by construction.
                if (is_start) {
            ...
            if (!is_goal) step.sphere_contact[i].reset();
        }
        return step;
    };
    ...
    for (int k = 0; k <= K; ++k) {
        const bool is_start = (k == 0);
        models_.push_back(std::make_unique<HandModel>(
            make_step_spec(k), wrist_pose, start_wrist_noise,
...
void HandTrajectoryPlanner::get_initial_values() {
    values_.clear();
...
void HandTrajectoryPlanner::build_graph() {
    graph_.resize(0);
    ...
    for (int k = 0; k <= K; ++k) {
        ...
        // HandModel::build_graph based on how each model was constructed).
        ...
        // Terminal per-digit tip-position goals (point-to-point). Soft priors on
        // each digit's tip node; only added when goal_positions is non-empty, so
        // legacy contact-as-goal / no-goal runs are unaffected.
        if (k == K && !config_.goal_positions.empty()) {
            const int n_digits = models_[K]->num_digits();
            if (static_cast<int>(config_.goal_positions.size()) != n_digits) {
                throw std::runtime_error(
                    "HandTrajectoryPlannerConfig::goal_positions size must "
                    "equal the number of digits when non-empty.");
            }
            auto goal_noise =
                noiseModel::Gaussian::Covariance(config_.goal_position_cov);
            for (int i = 0; i < n_digits; ++i) {
                graph_.add(PositionPriorFactor(
                    models_[K]->digit_tip_pose_key(i),
        ...
        if (k < K) {
            ...
            // Per-digit actuation GP (Eq 1.11) and optional displacement GP
            // (Eq 1.13).
            models_[k]->add_temporal_gp(
                graph_, *models_[k + 1],
                config_.gp_actuation_Qc, config_.gp_displacement_Qc, config_.dt);
        }
    }
}


void HandTrajectoryPlanner::extract_solution() {
    result_.trajectory.clear();
    result_.trajectory.reserve(models_.size());
    for (const auto& model : models_)
        result_.trajectory.push_back(model->get_state(values_, marginals_));
}


HandTrajectoryResult
HandTrajectoryPlanner::extract_trajectory_means_only(const Values& values) const {
    HandTrajectoryResult r;
    r.trajectory.reserve(models_.size());
    for (const auto& model : models_)
        r.trajectory.push_back(model->get_state_means_only(values));
    return r;
}


std::vector<HandTrajectoryResult>
HandTrajectoryPlanner::get_intermediate_solutions() const {
    std::vector<HandTrajectoryResult> out;
...
HandTrajectoryResult
HandTrajectoryPlanner::get_initial_solution() const {
    return extract_trajectory_means_only(initial_values_);
}


HandTrajectoryResult HandTrajectoryPlanner::plan(
