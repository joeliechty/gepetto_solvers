#pragma once

#include <gtsam/geometry/Pose3.h>
#include <gtsam/nonlinear/NonlinearFactorGraph.h>


using CosseratVelBase = gtsam::NoiseModelFactorN<
    gtsam::Pose3, gtsam::Vector6, gtsam::Pose3, gtsam::Vector6,gtsam::Vector6>;

class CosseratVelocityFactor: public CosseratVelBase {
    using CosseratVelBase::evaluateError;
public:
    CosseratVelocityFactor(
        gtsam::Key pose_prev_key,
        gtsam::Key v_prev_key,
        gtsam::Key pose_key,
        gtsam::Key v_key,
        gtsam::Key wrench_key,
        const gtsam::SharedNoiseModel& model,
        double dt);

    gtsam::Vector evaluateError(
        const gtsam::Pose3& pose_prev, 
        const gtsam::Vector6& v_prev,
        const gtsam::Pose3& pose, 
        const gtsam::Vector6& v,
        const gtsam::Vector6& wrench,
        gtsam::OptionalMatrixType H1, 
        gtsam::OptionalMatrixType H2, 
        gtsam::OptionalMatrixType H3, 
        gtsam::OptionalMatrixType H4,
        gtsam::OptionalMatrixType H5) const override;

    private:
        const double dt_;
};
