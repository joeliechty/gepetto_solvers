...
//
// AL budget: the outer Augmented Lagrangian loop is AMORTIZED ACROSS TICKS. With
// the constraint set unchanged between ticks and a warm start each time, mu and
// the multipliers keep ramping from one call to the next, so a small
// al_max_iterations (3-5) preserves the mu-homotopy the §1.5 notes call essential
// while keeping a single tick fast.
