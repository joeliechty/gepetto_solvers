...
bisected one constraint family at a time::
    ...
    python scripts/experimental/debug_ik_step_trace.py
    python scripts/experimental/debug_ik_step_trace.py \
        --table-contact --no-object-contact --no-object-collision
    python scripts/experimental/debug_ik_step_trace.py --table-contact
...
resolves to the installed binding rather than the in-tree ``.so``::
    ...
    python scripts/experimental/debug_ik_step_trace.py --log

    # add the C++ outer-loop stderr trace and the per-step inner-LM tail
    python scripts/experimental/debug_ik_step_trace.py --verbose --inner
