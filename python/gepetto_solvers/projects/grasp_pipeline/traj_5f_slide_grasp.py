...
Run (from the ``crest-sparse/`` directory):
    ...
    python scripts/traj_5f_slide_grasp.py big_sphere --no-viz
    # full slide-and-grasp: sliding starts at step 4
    python scripts/traj_5f_slide_grasp.py big_sphere --no-viz --k-touch 4
...
def plan_trajectory(args, configs):
    ...
    all_tip_wrenches = [gepetto_solvers.Vector6Gaussian(np.zeros(6), tip_wrench_cov)
                        for _ in configs]
    all_start = [gepetto_solvers.VectorXGaussian(start_mean, start_cov)
                 for _ in configs]
