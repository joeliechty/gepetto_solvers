...
onto the shared SDF object surface by its own hard SDF contact constraint. The
four fingers are mounted by rotating the known-good single-finger solve about the
object center (guaranteeing reach); the thumb is a shorter, pronated finger
opposing them from the radial side. See ``config.get_five_finger_grasp_configs``.
...
from .config import get_five_finger_grasp_configs, tip_node_index
...
def main():
    ...
    configs = get_five_finger_grasp_configs(OBJECT_CENTER)
    ...
    # flexor (index 5) loose so the optimizer drives contact. Tight per-finger
    # tip-wrench prior keeps the AL system well-conditioned (see memory
    # hand-wrench-prior-conditioning).
    ...
    for name, fm in zip(finger_names, solution.marginals.fingers):
        ...
        print(f"  [{name:>6}] tip {tip_pos}  |  surface gap {gap:+.5f} m  (target ~0)")
