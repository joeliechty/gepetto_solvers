...
Pure numpy -- no solver, no ROS, no hardware. Run from the crest-sparse root:

    python -m python.tests.tendon_hand.test_robot_plan_timing
...
def test_speed_stretches_time_and_leaves_the_path_alone():
    ...
    for k, (a, b) in enumerate(zip(fast.durations, slow.durations)):
    ...
    for k, (quick, slower) in enumerate(zip(fast.durations, slow.durations)):
