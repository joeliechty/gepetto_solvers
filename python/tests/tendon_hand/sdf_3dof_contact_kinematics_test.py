...
def main():
    ...
    parser.add_argument(
        "--use-hand-base", action="store_true",
        help="Enable the Section 4 hand-base reparameterization: node 0 becomes "
             "T_0 = T_base o T_offset (offset = Identity here, so the geometry and "
             "results match the legacy node-0 path).")
    ...
    # the legacy node-0 path.
    if args.use_hand_base:
        config.use_hand_base = True
        config.hand_base_offset = np.eye(4)
