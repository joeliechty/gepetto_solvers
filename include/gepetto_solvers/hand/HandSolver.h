...
struct HandSolverConfig {
    ...
    // HandTrajectoryPlannerConfig::goal_positions. Because these are soft
    ...
    // HandModel::get_initial_values, unchanged. Set, and the solve begins
    ...
    std::optional<HandState> initial_state;
...
// floating wrist base. Thin SolverBase wrapper around HandModel; when any
...
public:
    ...
    int num_digits() const { return hand_->num_digits(); }

    // Kept as the name every Python caller and script already uses; a "digit"
    // and a "finger" are the same thing to a caller counting them.
    int num_fingers() const { return hand_->num_digits(); }
    ...
    std::vector<HandState> get_intermediate_solutions() const;
    ...
    HandState get_initial_solution() const;
